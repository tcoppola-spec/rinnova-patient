...
- `color_key` TEXT **NOT NULL** — the treatment category. Injectables: `xeomin` (purple, any neurotoxin), `radiesse` (magenta), `radiesse-light` (coral, diluted Radiesse), `rha` (orange, any HA filler). Non-injectables (added Aug 2026): `energy` (teal — Ultherapy, RF, ultrasound), `light` (magenta-red — LED / red-light therapy, phototherapy), `resurfacing` (rose — laser resurfacing, peels, microneedling), and `other` (neutral grey — anything administered we can't categorise, incl. body treatments while body is parked). No DB CHECK enforces this. **The single source of truth is now `src/treatmentColors.js`** (colour + `mark` type + label), NOT a map in `FaceDiagram` — that map used to be copy-pasted into three files and the colour IS the clinical category, so drift mislabels a record. Keep the `--energy`/`--resurfacing` tokens and the `.treatment-dot-*` classes in step with it.

  **⚠️ POINT vs FIELD — the rule the non-injectable feature turns on.** Each category has a `mark`: injectables are `point` (a DOT — filler went to a spot), `energy`/`light`/`resurfacing`/`other` are `field` (a soft HALO — a laser covers a zone whose boundary we usually don't know; a dot would claim false precision). **The treatment TYPE decides point-vs-field; the region decides where** — the same "cheeks" is a dot for filler and a halo for a laser. Halos render over the line art but UNDER the dots (injectable points stay crisp on the wash), via a `halos` prop on `FaceDiagram` and a radial gradient that fades to transparent. A **"Full face"** area (common for resurfacing) draws ONE large centred halo (`FULL_FACE` in `faceGeometry.js`) from the NAME, not a mark at a point; it resolves to face-centre in `faceCoordinates.js` so it's a *placed* region (never a false "couldn't map"). Bare `"face"` is deliberately left unresolvable in both places — a generic "Face" area was the old receipt-fabrication signal. `saveVisit`'s blank-colour fallback is `'other'`, NOT `'xeomin'` — an uncategorised treatment must never be silently painted as a neurotoxin. **`other` is a FIELD** (grey halo), not a point: a non-injectable administered treatment covers an area, so with a location it reads as a soft zone, never a stray pinpoint dot. This is why LED/light therapy (→ `energy`) and any uncategorised facial treatment draw a halo. Learned from a real LED red-light log that showed nothing (it was `other`+point at the time). **Body is parked:** facial energy/resurfacing only; body treatments capture as `other` with no location (no body map yet).
